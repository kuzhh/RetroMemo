...
void shuffleCards(tBoard *board) {
  ...
  for (int i = 0; i < board->totalCards; i++) {
    ...
    // Ahora las cartas están dentro del tVector, entonces accedemos con
    // vector_get.
...
int boardInit(tBoard *board, int rows, int cols) {
  ...
  if (vector_init(&board->cards, sizeof(tCard),
                  board->totalCards)) // ADD: crea vector para guardar tCard
  {
...
void boardDestroy(tBoard *board) {
  ...
  for (int i = 0; i < board->totalCards; i++) {
    tCard *c =
        (tCard *)vector_get(&board->cards, i); // ADD: obtengo carta del vector
    if (!c)
      continue;
