...
int screenInitialize(tScreen * screen,
...
int mainMenuInit(tMainMenu * menu, SDL_Renderer * renderer, tAssets * assets) {
    ...
    menu -> btnSP.rect =
        (SDL_Rect) {
            centerX - (btnWidth / 2), centerY + 50, btnWidth, btnHeight
        };
    ...
    if (lblCreate( & menu -> lblSP, renderer, assets -> font, "Singleplayer", white) !=
        OK)
        return SDL_ERR;
    ...
    if (lblCreate( & menu -> lblMP, renderer, assets -> font, "Multiplayer", white) !=
        OK)
        return SDL_ERR;
    menu -> lblMP.rect.x =
    ...
    if (lblCreate( & menu -> lblExit, renderer, assets -> font, "Exit game", white) !=
        OK)
        return SDL_ERR;
    menu -> lblExit.rect.x =
...
int singlePlayerInit(tSinglePlayerScreen * single, SDL_Renderer * renderer,
    ...
    single -> logoRect = (SDL_Rect) {
        centerX - (LOGO_W / 2), 10, LOGO_W, LOGO_H
    };

    single -> btnBack.rect = (SDL_Rect) {
        20,
        20,
        80,
        80
    ...
    single -> btnInputName.rect =
        (SDL_Rect) {
            centerX - (BTN_W / 2), centerY + 50, BTN_W, BTN_H
        };
...
int multiPlayerInit(tMultiplayerScreen * multi, SDL_Renderer * renderer,
    ...
    multi -> logoRect = (SDL_Rect) {
        centerX - (LOGO_W / 2), 10, LOGO_W, LOGO_H
    };
    multi -> btnBack.rect = (SDL_Rect) {
        20,
        20,
        80,
        80
    ...
    // Inputs Positions
    multi -> btnInputName1.rect = (SDL_Rect) {
        (centerX / 2) - (btnWidth / 2),
    ...
    multi -> btnInputName2.rect =
        (SDL_Rect) {
            centerX + (centerX / 2) - (btnWidth / 2), centerY + 50,
                btnWidth, btnHeight
        };
    ...
    multi -> btnContinuar.rect =
        (SDL_Rect) {
            centerX - (btnWidth / 2), centerY + 170, btnWidth, btnHeight
        };
...
void multiPlayerUpdate(tMultiplayerScreen * multi, tInput * input,
    ...
    if (input -> mouseReleased) {
        ...
        if (pointInRect(input -> mouseX, input -> mouseY, & multi -> btnInputName1.rect)) {
            if (!multi -> textActive1) {
                ...
                // CORRECCION: Recalcular longitud real para que el backspace sepa dónde
                // está
    ...
    if (input -> mouseReleased &&
        ...
        if (strlen(multi -> textInput1) > 0)
            strcpy(game -> players[0].namePlayer, multi -> textInput1);
        else
            strcpy(game -> players[0].namePlayer, "Player 1");

        if (strlen(multi -> textInput2) > 0)
...
int playMPInit(tPlayMPScreen *MP, SDL_Renderer *renderer, tAssets *assets,
    ...
    // Ahora sí: ya existe lblPlayerScore[1] y su rect.w es válido
    MP->lblPlayerName[1].rect.x = SCREEN_WIDTH - MP->lblPlayerName[1].rect.w - 100;
...
void playMPRender(SDL_Renderer *renderer, tPlayMPScreen *MP, tAssets *assets,
    ...
    for (int i = 0; i < game->playerCount; i++) {
        ...
        // ----- PLAYER NAME -----
        if (i == active) {
            SDL_SetTextureColorMod(MP->lblPlayerName[i].texture,
        ...
        // ----- PLAYER SCORE -----
        if (i == active) {
            SDL_SetTextureColorMod(MP->lblPlayerScore[i].texture,
                                glowColor.r, glowColor.g, glowColor.b);
...
int playSPExitInit(tPlaySPScreenExit * SP, SDL_Renderer * renderer,
    ...
    if (lblCreate( & SP -> lblExit, renderer, assets -> font, "Exit game", white) != OK)
        return SDL_ERR;
...
int playMPExitInit(tPlayMPScreenExit * MP, SDL_Renderer * renderer,
    ...
    MP -> scoreBoxRectP2 =
        (SDL_Rect) {
            SCREEN_WIDTH - marginX - boxW, boxY, boxW, boxH
        };
    ...
    if (lblCreate( & MP -> lblExit, renderer, assets -> font, "Exit game", white) != OK)
        return SDL_ERR;
    MP -> lblExit.rect.x =
