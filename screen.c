...
int mainMenuInit(tMainMenu *menu, SDL_Renderer *renderer, tAssets *assets)
{
    ...
    // Botones
    menu->btnSP.rect = (SDL_Rect){centerX - (btnWidth / 2),
                                  centerY + 50,
                                  btnWidth,
                                  btnHeight};
                      ...
                      tInput *input, ScreenType *currentScreen)
{
    ...
    if (input->mouseReleased)
    {
        ...
        if (pointInRect(input->mouseX, input->mouseY, &SP->btnPantallaInicio.rect))
        {
            *currentScreen = SCREEN_MAIN;
                      ...
                      tAssets *assets, tBoard *board)
{
    ...
    SDL_RenderCopy(renderer, SP->lblTryAgain.texture, NULL, &SP->lblTryAgain.rect);
    SDL_RenderCopy(renderer, SP->lblPantallaInicio.texture, NULL, &SP->lblPantallaInicio.rect);
    SDL_RenderCopy(renderer, SP->lblExit.texture, NULL, &SP->lblExit.rect);
    SDL_RenderCopy(renderer, SP->lblPlayerName.texture, NULL, &SP->lblPlayerName.rect);
    SDL_RenderCopy(renderer, SP->lblPlayerScore.texture, NULL, &SP->lblPlayerScore.rect);
    SDL_RenderCopy(renderer, SP->lblPlayerMoves.texture, NULL, &SP->lblPlayerMoves.rect);
}
                      ...
                      tInput *input, ScreenType *currentScreen)
{
    ...
    if (input->mouseReleased)
    {
        ...
        if (pointInRect(input->mouseX, input->mouseY, &MP->btnPantallaInicio.rect))
        {
            *currentScreen = SCREEN_MAIN;
                      ...
                      tAssets *assets, tBoard *board, tGame *game)
{
    ...
    SDL_RenderCopy(renderer, MP->lblTryAgain.texture, NULL, &MP->lblTryAgain.rect);
